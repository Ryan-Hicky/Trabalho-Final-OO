...
public class Pedido {
    ...
    public void adicionarItem(Produto p, int quantidde) throws EstoqueInsuficienteException{
        if (p.QuantidadeEstoque() < quantidde){
            throw new EstoqueInsuficienteException("Estoque insuficiente para o produto" + p.getNome());
        }
        ItemPedido novoItem = new ItemPedido(p, quantidde);
        itens.add(novoItem);
    }
